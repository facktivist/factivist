...
describe('EMAIL_REGEX', () => {
  it.each(['a@b.co', 'user.name+tag@example.com', 'first.last@sub.domain.io'])(
    'matches valid email %s',
    (email) => {
      expect(EMAIL_REGEX.test(email)).toBe(true);
    },
  );

  it.each(['', 'no-at-sign', 'a@b', 'spaces in@email.com', '@nouser.com', 'user@'])(
...
describe('SLUG_REGEX', () => {
  it.each(['my-post', 'factivist', 'factivist-2026', 'a1-b2-c3'])(
    'matches valid slug %s',
    (slug) => {
      expect(SLUG_REGEX.test(slug)).toBe(true);
    },
  );

  it.each(['', '-bad', 'bad-', 'Bad-Case', 'double--hyphen', 'has space', 'has_underscore'])(
