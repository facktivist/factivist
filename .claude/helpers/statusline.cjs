        ...
        if (CWD === projectPath || CWD.startsWith(projectPath + '/')) {
          ...
          if (usage) {
            ...
            if (ids.length > 0) {
              ...
              for (const id of ids) {
                const ts =
                  usage[id] && usage[id].lastUsedAt ? new Date(usage[id].lastUsedAt).getTime() : 0
                if (ts > latest) {
                  latest = ts
                  modelId = id
                }
...
function getV3Progress() {
  ...
  const realPatterns =
    Number.isFinite(learning.patterns) && learning.patterns >= 0 && learning.patterns < 1_000_000
      ? learning.patterns
      : 0
...
function generateStatusline() {
  ...
  try {
    ...
    for (const p of pkgPaths) {
      ...
      try {
        ...
        /* malformed package.json — try next */
      }
    }
  } catch {
  ...
  if (git.gitBranch) {
    header +=
      '  ' + c.dim + '\u2502' + c.reset + '  ' + c.brightBlue + '\u23C7 ' + git.gitBranch + c.reset
  ...
  if (duration)
    header += '  ' + c.dim + '\u2502' + c.reset + '  ' + c.cyan + '\u23F1 ' + duration + c.reset
