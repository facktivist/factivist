...
function buildTitlePage(data) {
  return [
    ...
    new Paragraph({
      children: [
        new TextRun({
          text: 'Supplementary Reading List',
          bold: false,
          size: 28,
          color: ACCENT_BLUE,
        }),
    ...
    data.courseSubtitle
      ? new Paragraph({
          children: [
            new TextRun({ text: data.courseSubtitle, italics: true, size: 22, color: DARK_GRAY }),
          ],
          alignment: AlignmentType.CENTER,
          spacing: { after: 800 },
        })
    ...
    new Paragraph({
      children: [
        new TextRun({
          text: `Generated: ${data.generatedDate || new Date().toISOString().split('T')[0]}`,
          size: 18,
          color: GRAY,
        }),
      ],
    ...
    new Paragraph({
      children: [
        new TextRun({
          text: `Year range: ${data.yearRange || 'last 2 years'}`,
          size: 18,
          color: GRAY,
        }),
      ],
...
function buildIntroSection(data) {
  const introText =
    data.introText ||
    'This supplementary reading list collects recent peer-reviewed research relevant to each section of the course. Each entry includes a plain-language summary calibrated to the course audience and a discussion question tied to the course learning outcomes.'
  return [
    new Paragraph({
    ...
    new Paragraph({
      children: [
        ...
        new TextRun({
          text: ' academic search. URLs in this document link directly to Consensus paper records.',
          size: 20,
        }),
      ],
      spacing: { after: 400 },
...
function buildSection(section, sectionIndex) {
  const elements = [
    new Paragraph({
      ...
      children: [
        new TextRun({
          text: `${sectionIndex}. ${section.heading}`,
          color: NAVY,
          bold: true,
          size: 28,
        }),
      ...
      new Paragraph({
        children: [
          ...
          new ExternalHyperlink({
            link: paper.url,
            children: [
              new TextRun({ text: paper.title, style: 'Hyperlink', size: 22, bold: true }),
            ],
          }),
        ],
        spacing: { after: 60 },
      ...
      elements.push(
        new Paragraph({
          children: [
            ...
            new TextRun({
              text: `"${sd.query}" → ${sd.papersReturned || 0} returned, ${sd.papersSelected || 0} selected (${sd.status || 'OK'})`,
              size: 18,
            }),
          ],
          spacing: { after: 40 },
  ...
  return new Footer({
    children: [
      new Paragraph({
        children: [
          new TextRun({
            text: `${data.courseTitle} — Supplementary Reading List`,
            size: 16,
            color: GRAY,
          }),
        ],
...
function main() {
  ...
  const sections = data.sections.flatMap((s, i) => buildSection(s, i + 1))
  ...
  Packer.toBuffer(doc)
    .then((buffer) => {
      ...
      console.log(
        `Generated: ${opts.output} (${buffer.length} bytes, ${data.sections.length} sections, ${data.sections.reduce((sum, s) => sum + s.papers.length, 0)} papers)`,
      )
    })
    .catch((e) => {
