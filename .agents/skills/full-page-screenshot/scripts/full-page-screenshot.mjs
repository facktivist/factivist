...
async function connectChrome() {
  ...
  await new Promise((resolve, reject) => {
    const timer = setTimeout(() => {
      ws.close()
      reject(
        new Error('WebSocket connection timeout (10s) — browser WebSocket may be held by proxy'),
      )
    }, 10000)
    ws.onopen = () => {
      clearTimeout(timer)
...
async function captureFullPage({ sid, outputFile, width, devicePixelRatio, css }) {
  ...
  if (finalH <= TILE_THRESHOLD) {
    ...
    console.log(
      `Saved: ${outputFile} (${width * devicePixelRatio}x${finalH * devicePixelRatio}px, ${mb} MB)`,
    )
  } else {
    ...
    try {
      const result = execSync(`python3 "${stitchScriptPath}"`, {
        encoding: 'utf-8',
        timeout: 60000,
      })
    ...
    try {
      fs.unlinkSync(stitchScriptPath)
    } catch {}
...
async function modeUrl() {
  ...
  if (connected) {
    ...
    try {
      ...
      if (createdTargetId) {
        try {
          await send('Target.closeTarget', { targetId: createdTargetId })
        } catch {}
      }
      closeWs()
    }
  ...
  try {
    ...
    try {
      await fetch(`${PROXY_URL}/close?target=${targetId}`, { signal: AbortSignal.timeout(5000) })
    } catch {}
  }
}
...
async function proxyScreenshot(targetId, filePath) {
  const resp = await fetch(
    `${PROXY_URL}/screenshot?target=${targetId}&file=${encodeURIComponent(filePath)}`,
    {
...
async function captureViaProxy(targetId, outputFile, width, devicePixelRatio) {
  ...
  const containerInfo = await proxyEval(
    targetId,
    `(() => {
    const containers = [];
    for (const el of document.querySelectorAll('*')) {
  ...
  const pageInfo = await proxyEval(
    targetId,
    `({
  ...
  const mainContainer =
    containerInfo && containerInfo.length > 0
  ...
  if (mainContainer && mainContainer.scrollHeight > vpH) {
    ...
    console.log(
      `Scroll container: ${mainContainer.selector} (${mainContainer.scrollHeight}px content in ${mainContainer.clientHeight}px view)`,
    )
    ...
    if (tiles.length === 1) {
      ...
      const containerOffset = await proxyEval(
        targetId,
        `(() => {
        const containers = [];
        for (const el of document.querySelectorAll('*')) {
...
async function modeTarget() {
  ...
  if (!connected) {
    ...
    if (!proxyUp) {
      console.error(
        'Cannot connect to Chrome (browser WebSocket unavailable) and no proxy running.',
      )
      process.exit(1)
    }
    await captureViaProxy(targetId, outputFile, vpWidth, dpr)
