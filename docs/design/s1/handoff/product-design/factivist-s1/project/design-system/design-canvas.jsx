  ...
  React.useLayoutEffect(() => {
    ...
    try {
      ...
      if (s && Number.isFinite(s.x) && Number.isFinite(s.y) && Number.isFinite(s.scale)) {
        ...
        apply();
        // Defensive: if the restored viewport places no content on-screen
        // (e.g. the canvas grew after a generation, leaving the saved pan
        // past the new end), reset to the default origin so the user isn't
        // stuck looking at empty space. Checked on next frame so DOM has
        // mounted.
        requestAnimationFrame(() => {
          const vp = vpRef.current;
          if (!vp) return;
          const vr = vp.getBoundingClientRect();
          const sections = vp.querySelectorAll('[data-dc-section]');
          if (!sections.length) return;
          const anyVisible = Array.from(sections).some((el) => {
