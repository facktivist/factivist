...
| **T**ampering | Direct UPDATE bypassing nullifier guard | DBA action or compromised key. | RLS ON for every table; `audit_log` is append-only via `INSERT`-only role; daily Drizzle migration diff verified in CI. | [[ADR-001]] |
...
| **E**oP | RLS misconfiguration grants anon read on `audit_log` | Migration mistake. | RLS policy unit tests in `packages/db/__tests__/rls.test.ts` for every table; CI fails on missing policy. | [[ADR-001]] |
