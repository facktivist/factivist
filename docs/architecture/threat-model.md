...
| **E**oP | RLS misconfiguration grants anon read on `audit_log` | Migration mistake. | Migration `0004_enable_rls.sql` declares the policy set; `kg-drift-guard.yml` workflow + Drizzle snapshot diff fail CI on any policy regression. RLS coverage test (`packages/db/__tests__/rls.test.ts`) deferred to Phase 9 §5.9 follow-up after first migration applies in production. | [[ADR-001]] |
